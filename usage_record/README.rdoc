...
This project contains Java classes to produce an XML document compliant with the Usage Record specification GFD.098 [1]. This project was created using IntelliJ IDEA 7.0.5, so you can use the .ipr, iml etc. files with this IntelliJ if necessary.
...
5. The Usage Record in XML format can be obtained at any time by calling the +toXml()+ method. If you want a pretty XML output (i.e., with line breaks and indentation) call the +toPrettyXML()+.
...
2. *toRUS*. Want to implement a method that sends the usage record to a RUS given the EPR or URL of the service accepting records for persistant storage

3. *Better JavaDoc comments*. The parameters/arguments need relating to the definitions in the GFD.098 specification so there is no misunderstanding what they mean
